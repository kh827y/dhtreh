...
- `GET /portal/analytics/referral` — параметры `period=yesterday|week|month|quarter|year` либо пара `from`/`to` (ISO‑даты, фронт передаёт `YYYY-MM-DD`, backend нормализует к началу/концу суток в таймзоне мерчанта). Ответ:
  ...
  - `purchasedViaReferral` — сколько приглашённых клиентов совершили свою первую покупку в выбранном периоде (`Referral.completedAt` в диапазоне), независимо от даты регистрации.
  ...
  - `topReferrers[]` — до 20 записей `{ rank, customerId, name, invited }`, где `invited` — число клиентов, которые указали код конкретного пользователя в заданном периоде (по `Referral.activatedAt`).
...
- Нумерация R/F/M идёт по возрастанию: `1` соответствует самым «тёплым» клиентам, `5` — наименее активным.
...
Item-level формат позиций для CALCULATE-ACTION/CALCULATE-BONUS/BONUS: `items[]` с полями `id_product`, `categoryId?`, `name?`, `qty`, `price`, `base_price?`, `allow_earn_and_pay?`, `actions?`, `action_names?`. Маппинг по паре (id_product, categoryId) или внутреннему `productId`; `externalProvider`/`categoryExternalId`/`sku`/`barcode` больше не используются. Баллы/списание распределяются по позициям, учитываются акции по товарам (множители, N-й бесплатно, акционная цена).
