...
export function useMiniappAuth(defaultMerchant: string) {
  ...
  useEffect(() => {
    telegramUserIdRef.current = telegramUserId;
  }, [telegramUserId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const resolvedTelegramUserId =
        initDataUserId || telegramUserIdRef.current || getTelegramUserId();
      if (resolvedTelegramUserId && resolvedTelegramUserId !== telegramUserIdRef.current) {
      ...
      const fallbackMerchant = ctxMerchant || defaultMerchant || merchantIdRef.current;
      if (ctxMerchant && ctxMerchant !== merchantIdRef.current) setMerchantId(ctxMerchant);
      else if (
        !ctxMerchant &&
        defaultMerchant &&
        defaultMerchant !== merchantIdRef.current
      )
