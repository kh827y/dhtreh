...
- **[SEC][Auth] `requireStaffKey`/`requireBridgeSig` фактически не защищают кассовые операции (`/loyalty/quote|commit|refund`)**
  - **Риск**: клиент/атакующий может дергать `quote` с произвольной суммой → получить `holdId` → сделать `commit` и начислить себе баллы без реальной покупки (и/или инициировать списания/возвраты). Это прямой абуз баллов и финансовые потери.
  - **Причина**:
    - **`CashierGuard` не учитывает** `merchantSettings.requireStaffKey/requireBridgeSig` (флаги не используются в guard’е).
    - **`CashierGuard` разрешает write-операции без auth**, если нет `cashier_session`/`teleauth`/`X-Bridge-Signature`, но в запросе есть `merchantId` (fallback `return Boolean(merchantIdFromRequest)`). Это похоже на legacy/отладочный режим и должно быть убрано.
    - В `LoyaltyController` проверка bridge‑подписи для `commit/refund` обёрнута в `try { ... } catch {}` — ошибки (включая `UnauthorizedException`) глушатся.
    - Даже при `requireBridgeSig=true` подпись не является обязательной (проверяется только если заголовок присутствует).
  - **Где**: `api/src/guards/cashier.guard.ts`, `api/src/loyalty/loyalty.controller.ts` (`commit`, `refund`).
  - **Что сделать (без overengineering)**:
    - Для кассовых/изменяющих баланс операций (`quote`, `commit`, `refund`, возможно `cancel`) требовать **минимум один** фактор: `cashier_session` (cookie), **или** валидный `X-Staff-Key`, **или** валидный `X-Bridge-Signature`.
    - Убрать/закрыть fallback “пропуск по одному `merchantId`” для write-операций.
    - Если включён `requireBridgeSig=true` — сделать подпись обязательной (нет заголовка → 401) и **не глушить** исключения.
    - Если включён `requireStaffKey` — требовать `X-Staff-Key`/сессию кассира.
    - Добавить автотесты на запрет доступа без auth.

- **[SEC][PII] Публичный доступ к кассовым эндпоинтам с ПДн/метриками из-за fallback в `CashierGuard`**
  - **Риск**: любой внешний клиент может получить чувствительные данные (ПДн клиентов и внутреннюю аналитику/историю операций) **без авторизации**, просто передав `merchantId` (и часто `outletId`). Это критично для продакшена.
  - **Проявления**:
    - `GET /loyalty/cashier/outlet-transactions?merchantId=...&outletId=...` — возвращает транзакции по точке, включая `customerName` (в т.ч. телефон/email), `receiptNumber`, суммы, `staffName`.
    - `POST /loyalty/cashier/customer` — позволяет резолвить QR → получить `customerId`/имя/баланс (дальше это можно связать с другими уязвимостями).
    - `GET /loyalty/cashier/leaderboard` — утечка мотивационной статистики/данных персонала.
  - **Причина**: эти роуты не находятся в `telegramProtectedPaths`, а при отсутствии `cashier_session`/`X-Staff-Key`/`X-Bridge-Signature` guard делает `return Boolean(merchantIdFromRequest)`.
  - **Где**: `api/src/guards/cashier.guard.ts`, `api/src/loyalty/loyalty.controller.ts` (cashier endpoints), `api/src/loyalty/loyalty.service.ts` (`outletTransactions` выбирает `customer.phone/email`).
  - **Что сделать**: требовать `cashier_session` **или** `X-Staff-Key` (и/или `X-Bridge-Signature`) для всех `cashier/*` эндпоинтов; убрать поддержку query‑доступа без сессии; убрать fallback.

- **[SEC][Portal] `merchant-portal` позволяет закрепить чужую сессию + open redirect через `/api/session/accept-token`**
  - **Риск**:
    - **Session fixation**: злоумышленник может “подсунуть” жертве свой `portal_jwt` → жертва будет работать в портале под чужим аккаунтом/мерчантом (ошибочные настройки, утечки через скриншоты/экспорт, поддержка “не понимает что происходит”).
    - **Open redirect**: можно увести пользователя на внешний домен (`https://evil.tld`) под видом легитимного перехода (фишинг/перехват доверия).
  - **Причина**:
    - `GET/POST` принимает `token` из query/body и без проверок пишет его в cookie `portal_jwt`.
    - `redirect` не валидируется и допускает абсолютные URL (через `new URL(redirect, origin)`).
    - Роут публичный (добавлен в `pubPaths`) и доступен без существующей сессии.
  - **Где**: `merchant-portal/app/api/session/accept-token/route.ts`, `merchant-portal/middleware.ts`.
  - **Что сделать (без overengineering)**:
    - если это legacy — удалить роут и убрать из `pubPaths`;
    - иначе: принимать не “готовый JWT”, а одноразовый короткоживущий `code` и обменивать его на токен через backend; `redirect` ограничить только относительными путями (`/...`) по тому же принципу, что `safeRedirectPath()` в `/api/session/refresh`.
...
- **[INFRA][Build] Production‑сборки UI сейчас падают (невозможно задеплоить `admin`/`merchant-portal`/`cashier`/`miniapp`)**
  - **Риск**: интерфейсы недоступны в проде → невозможно управлять мерчантами/кассой/miniapp, релизы блокируются.
  - **Проявления (локально)**:
    - `pnpm -C merchant-portal build` падает на ESLint‑ошибках (`react/no-unescaped-entities`, `prefer-const` и др.).
    - `pnpm -C admin build` падает на `@typescript-eslint/no-explicit-any` и др.
    - `pnpm -C cashier build` падает на `@typescript-eslint/no-explicit-any`.
    - `pnpm -C miniapp build` падает на TS‑ошибке (`string | null` → `string`) в `miniapp/app/page.tsx`.
  - **Что сделать (без overengineering)**: либо исправить ошибки линтера/типов, либо временно отключить ESLint‑гейтинг на build (`next.config.*: eslint.ignoreDuringBuilds=true`) чтобы разблокировать деплой (а затем вернуть обратно).

- **[INFRA][Compose] В `docker-compose.production.yml` отсутствует сервис `merchant-portal` (и в проекте нет `merchant-portal/Dockerfile`)**
  - **Риск**: мерчантам нечем пользоваться в проде (настройки/CRM/промо/аналитика недоступны) — фактически блокер запуска продукта.
  - **Где**: `docker-compose.production.yml`, отсутствие `merchant-portal/Dockerfile`.
  - **Что сделать (минимально)**:
    - добавить `merchant-portal` как отдельный сервис в production compose (с Traefik router, как у `admin/cashier/miniapp`);
    - добавить `merchant-portal/Dockerfile` (по аналогии с `admin/miniapp`) и `.dockerignore`.

- **[SEC][Cashier][LEGACY] `cashierPassword9` хранится в открытом виде + есть публичные эндпоинты на его основе**
  - **Риск**: при утечке БД/логов/скриншотов — простой захват кассового доступа (и дальнейший абуз баллов через кассовые операции/выдачу staff‑токенов).
  - **Причина**: пароль сравнивается как plaintext (`m.cashierPassword9 !== password9`), поле лежит в таблице `Merchant`; при этом основной кассовый UI использует activation‑codes и не нуждается в этом механизме.
  - **Где**: `api/prisma/schema.prisma` (`Merchant.cashierPassword9`), `api/src/merchants/merchants.service.ts` (`authenticateCashier`), `api/src/loyalty/loyalty.controller.ts` (`POST /loyalty/cashier/login`, `POST /loyalty/cashier/staff-token`), `api/src/merchant-panel/dto/cashier.dto.ts` (DTO включает `password`).
  - **Что сделать (как вы просили — без лишней сложности)**: принять одно решение и подчистить:
    - **Рекомендуемый путь**: считать это legacy и удалить `cashierPassword9` + эндпоинты `/cashier/login` и `/cashier/staff-token`, оставив activation‑codes + `cashier_session`.
    - Если всё-таки нужно: хранить только хэш (и показывать пароль **только при ротации**), добавить блокировки/лимиты по попыткам.

- **[REFUND][Data] `receiptNumber` не уникален, но используется для поиска чека → риск возврата “не того” заказа**
  - **Риск**: кассир делает refund по номеру чека и система выбирает другой `Receipt` (особенно при нескольких точках/сбросе нумерации) → неверное восстановление/списание баллов и конфликт с клиентом.
  - **Причина**: `Receipt.receiptNumber` не имеет уникального ограничения, а код делает `findFirst(where: { merchantId, receiptNumber })` без `outletId`/периода.
  - **Где**: `api/prisma/schema.prisma` (`Receipt.receiptNumber`), `api/src/loyalty/loyalty.controller.ts` (`refund`: fallback по `receiptNumber`), `api/src/guards/cashier.guard.ts` (refund‑контекст тоже резолвит `orderId` по `receiptNumber`).
  - **Что сделать**: убрать refund‑fallback по `receiptNumber` и требовать `invoice_num/order_id` (или, если очень нужно — принимать `receiptNumber` только вместе с `outletId` и/или гарантировать уникальность на уровне БД по выбранному ключу).

- **[SEC][Telegram] `telegramStartParamRequired` фактически не enforced (fail‑open)**
  - **Риск**: если используется один Telegram‑бот на несколько мерчантов (`TELEGRAM_BOT_TOKEN` общий), пользователь может получить `customerId`/создать профиль в “чужом” мерчанте простым вызовом `POST /loyalty/teleauth` с другим `merchantId` → cross‑tenant абуз/утечки.
  - **Причина**: в `teleauth()` проверка `merchantId mismatch with start_param` завернута в `try { ... } catch {}` и ошибки глушатся; строгая проверка выполняется только для JWT‑подобного `start_param`, но не для обычной строки.
  - **Где**: `api/src/loyalty/loyalty.controller.ts` (`POST /loyalty/teleauth`).
  - **Что сделать**: если включён `telegramStartParamRequired=true` — **жёстко** требовать `start_param` и совпадение с `merchantId` (без `catch {}`), либо отказаться от “общего бота” и хранить отдельные токены на мерчанта.

- **[SEC][TENANT] Нарушение изоляции мерчантов в portal: доступ/изменение чужих клиентов по `customerId`**
  - **Риск**: мерчант/сотрудник мерчанта **А**, зная/подбирая `customerId`, может работать с клиентами другой компании **Б**.
    - **Утечка ПДн**: получение `phone/email/name` и др. полей чужого клиента через `GET /portal/customers/:customerId`.
    - **Порча данных**: изменение профиля чужого клиента через `PUT /portal/customers/:customerId`.
    - **Загрязнение учёта**: создание/изменение `Wallet`/`Transaction` для чужого `customerId` с `merchantId` текущего мерчанта (возможна запись несогласованных связей, т.к. в схеме нет жёсткой связки `wallet.merchantId == customer.merchantId`).
  - **Причина**:
    - `PortalCustomersService.get/update/ensureOperationAllowed` читает клиента по `findUnique({ where: { id: customerId } })` и **не проверяет** `customer.merchantId === merchantId`.
    - `ensureOperationAllowed()` при mismatch делает `return` (то есть **не запрещает** операции начисления/списания).
    - Аналогично `CustomerAudiencesService.getCustomer()` использует `findFirst(where: { id: customerId })` без tenant‑проверки.
  - **Где**:
    - `api/src/portal/customers.service.ts` (`get`, `update`, `ensureOperationAllowed`, `accrueManual/redeemManual/issueComplimentary`).
    - `api/src/customer-audiences/customer-audiences.service.ts` (`getCustomer`).
  - **Что сделать (просто)**:
    - Во всех portal/CRM сервисах получать клиента только по `{ id, merchantId }` (или после `findUnique` жёстко проверять `customer.merchantId`).
    - В `ensureOperationAllowed` при mismatch — бросать 404/403.

- **[SEC][QR] `POST /loyalty/qr` позволяет выписывать QR на произвольного клиента при передаче `initData` в body (без привязки к `customerId`)**
  - **Риск**: любой пользователь Telegram miniapp может выписать QR для чужого `customerId` (если ID утёк/получен) → потенциальный абуз/кража баллов и утечки (особенно в связке с текущей уязвимостью публичных `quote/commit/refund`).
  - **Причина**:
    - `CashierGuard` имеет спец-ветку `qrWithInitData` и пропускает `/loyalty/qr` при наличии `body.initData`.
    - В `mintQr()` подпись `initData` валидируется, но **userId/tgId из `initData` не сопоставляется с `dto.customerId`**.
  - **Где**: `api/src/guards/cashier.guard.ts` (`qrWithInitData`), `api/src/loyalty/loyalty.controller.ts` (`mintQr`).
  - **Что сделать (просто)**:
    - При `initData` **не принимать `customerId` от клиента**: брать `customerId` из Telegram контекста (tgId → Customer) и игнорировать/проверять входной `customerId`.
    - Или убрать body-параметр `initData` и использовать только `Authorization: tma <initData>` (чтобы `CashierGuard` мог жёстко сравнить `customerId`).

- **[SEC][QR] Опасный dev-fallback для подписи QR-токенов: `process.env.QR_JWT_SECRET || 'dev_change_me'`**
  - **Риск**: при ошибке деплоя/ENV в проде QR подпись становится предсказуемой → можно подделывать JWT‑QR и выдавать себя за любого клиента.
  - **Где**: `api/src/loyalty/loyalty.controller.ts` (`resolveFromToken`, `mintQr`), `api/src/integrations/integrations-loyalty.controller.ts` (`resolveFromToken`).
  - **Что сделать**: в production убрать fallback и падать при отсутствии/`dev_change_me` (env‑failfast + тест/healthcheck).
...
- **[SEC][Tokens] Ключи/секреты генерируются через `Math.random()` (не криптостойко)**
  - **Риск**: предсказуемые токены → компрометация авторизации/подписей и абуз баллов (особенно в multi-tenant: “понаблюдал свои токены” → попытка предсказать чужие).
  - **Проявления**:
    - `issueOutletBridgeSecret*()` генерирует `bridgeSecret/bridgeSecretNext` через `Math.random()` → подпись `X-Bridge-Signature` держится на слабом секрете.
    - `issueStaffToken()` генерирует staff API key (используется как `X-Staff-Key`) через `Math.random()`.
    - `PortalTelegramNotifyService.issueInvite()` генерирует invite token через `Math.random()`.
    - `GiftsService.randCode()` генерирует `GiftRedemption.code` через `Math.random()` (и не обрабатывает коллизии уникальности).
  - **Где**: `api/src/merchants/merchants.service.ts`, `api/src/portal/services/telegram-notify.service.ts`, `api/src/gifts/gifts.service.ts`.
  - **Что сделать (просто)**: заменить генерацию на `crypto.randomBytes(...)`/`crypto.randomUUID()` (и при необходимости оставить прежнюю длину/формат), а для `GiftRedemption.code` — добавить retry на уникальность.
...
- **[UI][Portal] Главная страница `merchant-portal` показывает неверный “статус настройки” (эндпоинт `/api/portal/setup-status`)**
  - **Риск**: онбординг/первичная настройка вводят в заблуждение — система может выглядеть “не настроенной” даже при корректной конфигурации (и наоборот), что мешает запуску.
  - **Причина**:
    - Дёргается несуществующий backend эндпоинт `GET /portal/settings/system` (в API есть `GET /portal/settings` и несколько под‑эндпоинтов).
    - `GET /portal/loyalty/mechanics` в backend возвращает **массив**, но код ожидает объект вида `mechanics.birthday.enabled` → `hasMechanics` почти всегда `false`.
    - Ошибки “глушатся” через `safeFetch()` (возвращает `null`) → UI часто показывает “не настроено” без понятной ошибки.
  - **Где**: `merchant-portal/app/api/portal/setup-status/route.ts`, `merchant-portal/app/page.tsx`, `api/src/loyalty-program/controllers/mechanics.controller.ts`.
  - **Что сделать**: переписать `/api/portal/setup-status` под реальные источники (`/portal/loyalty/tiers`, `/portal/outlets`, `/portal/staff`, `/portal/loyalty/mechanics` с проверкой `some(enabled)`), а ошибки показывать пользователю явно.

- **[ANALYTICS][Portal] Неверный период “Вчера” в статистике механик (используется `period=day` вместо `period=yesterday`)**
  - **Риск**: аналитика недостоверна (вместо “вчера” показывается “сегодня”) → неверные решения по эффективности механик.
  - **Причина**: в UI пресет `{ label: "Вчера", value: "day" }`, но backend `PortalController.computePeriod()` трактует `day` как “сегодня”, а “вчера” — это `yesterday`.
  - **Где**: `merchant-portal/app/loyalty/mechanics/auto-return/page.tsx`, `merchant-portal/app/loyalty/mechanics/birthday/page.tsx`, `api/src/portal/portal.controller.ts` (`computePeriod`).
  - **Что сделать**: заменить значение на `yesterday` (и при необходимости добавить отдельный пресет “Сегодня” = `day`).

- **[UI][Portal] В `merchant-portal` захардкожены ссылки на `localhost`**
  - **Риск**: в продакшене навигация ломается/вводит в заблуждение + светит внутренние URL.
  - **Где**: `merchant-portal/app/page.tsx` (`http://localhost:3004/...`), `merchant-portal/app/settings/page.tsx` (`http://localhost:3001/merchants`).
  - **Что сделать**: заменить на относительные пути внутри портала и/или конфиг через env (например `ADMIN_BASE_URL`), отключая ссылку если env не задан.

- **[UI][Portal] Переключатель “Реферальная программа” на странице механик может перетирать шаблон сообщения**
  - **Риск**: при включении/выключении рефералов тексты для приглашения/шаринга могут неожиданно сбрасываться на дефолты.
  - **Причина**: `merchant-portal/app/loyalty/mechanics/page.tsx` при `PUT /api/portal/referrals/program` не передаёт `shareMessage` (в ответе приходит `shareMessageTemplate`) и другие поля → backend подставляет fallback в `normalizeShareMessage()`.
  - **Где**: `merchant-portal/app/loyalty/mechanics/page.tsx`, `api/src/referral/referral.service.ts` (`updateProgramSettingsFromPortal`, `normalizeShareMessage`).
  - **Что сделать**: при toggle сохранять/передавать текущий `shareMessage` (из `shareMessageTemplate`) и остальные параметры, либо сделать отдельный endpoint “toggle enabled” без перезаписи настроек.
...
- **[UI][Import] Импорт клиентов: UI принимает только `.csv`, хотя backend поддерживает `.xlsx`**
  - **Риск**: пользователи будут пытаться загрузить Excel (и это логично по описанию/таблице колонок), но UI откажет → лишний friction и обращения в поддержку.
  - **Где**: `merchant-portal/app/customers/import/page.tsx` (валидация расширения/`accept`), `api/src/portal/portal.controller.ts` (`POST /portal/customers/import` поддерживает `.xlsx`).
  - **Что сделать**: разрешить `.xlsx` (и в `accept`, и в проверке), либо убрать упоминание Excel, если хотите поддерживать только CSV.
