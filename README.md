...
- Баллы R/F/M формируются по возрастанию: `1` — самые активные/лояльные клиенты, `5` — «остывшие» или потерянные.
...
- Если мерчант ещё не заводил уровни в портале, при первом обращении автоматически создаётся базовый уровень `Base`:
  - кэшбэк 3 % (`earnRateBps = 300`);
  - лимит списания 50 % (`redeemRateBps = 5000`);
  - минимальная сумма покупки 0;
  - стартовая группа (назначается всем при отсутствии явных назначений).
- Управление уровнями происходит через `LoyaltyTier` в портале (`/loyalty/mechanics/levels`).
  - Кнопка «Состав» открывает список клиентов уровня и перенаправляет в карточку клиента.
  - Скрытые группы (`isHidden`) не участвуют в авто‑повышениях и не отображаются в мини‑аппе до ручного назначения; продвижение в такие уровни выполняет админ или промокод.
- Ответ:
...
- API держит отдельное PG-подключение (`LoyaltyEventsService`), слушает канал и мгновенно закрывает long-poll `GET /loyalty/events/poll?merchantId=...&merchantCustomerId=...`, а при недоступности канала деградирует в периодическую проверку таблицы (без потери событий).
...
- События хранятся до тех пор, пока не будут выданы конкретному `merchantCustomerId`; повторная выдача исключена — по выдаче `deliveredAt` проставляется и запись игнорируется при следующем poll.
