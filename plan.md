...
## TODO 2025-12-08 — RFM recency auto
- [ ] Убрать дефолт 365 дней для Recency: по умолчанию авто-границы по квантилям, при этом поддержать ручной порог.
- [ ] Исключить клиентов с 0 покупок/0 суммой из расчёта квантилей R/F/M.
- [ ] Обновить UI/DTO/тесты под новые режимы и убедиться, что расчёт/аналитика согласованы.

## Хотфикс 2025-12-10 — Merchant Portal: история операций клиента
- [x] В карточке клиента разделение префиксов «Отменено администратором» и «Возврат» приведено к логике журнала операций (детекция admin по canceledBy/admin refund events).

## Хотфикс 2025-12-10 — Merchant Portal: Outlets cleanup
- [x] Упростить формы создания/редактирования точек: оставить только «Работает», «Название» и ссылки на отзывы.
- [x] В «Интеграциях» оставить только управление устройствами, очистить валидацию/пэйлоады от лишних полей.
- [x] Почистить список точек от дополнительной информации, оставив статус и название.

## Хотфикс 2025-12-10 — Merchant Portal: UI cleanup (analytics/audiences/reviews/telegram)
- [x] Убрать кнопку «Обновить» на странице /analytics.
- [x] Удалить текст про часовой пояс на /analytics/dynamics.
- [x] Переименовать блок «Сегменты» в «Аудитории» и привести модалки /audiences к стилю отзывов.
- [x] Очистить /reviews: оставить только «Предлагать поделиться отзывом», убрать уведомления/описание.
- [x] Перенести порог оценки для уведомлений в /settings/telegram при «Оповещать о новых отзывах» (персонально для сотрудника).

## Хотфикс 2025-12-10 — Cashier: история чеков по точке
- [x] В кассе вкладка «История» должна показывать все чеки/возвраты по текущей точке (из сессии кассира), а не по выбранному клиенту.
- [x] Добавить API/контроллер для выборки транзакций по outletId (без customerId), исключить админские отмены.
- [x] Обновить UI вкладки «История»: читаемые карточки (тип, сумма, номер чека, дата/время), без отображения ID клиентов/сотрудников.
- [x] При allowSameReceipt агрегировать начисление+списание и возвраты в одну запись чека; показывать сумму покупки, сотрудника и клиента.

## Хотфикс 2025-12-08 — Merchant Portal: аналитика
- [x] Staff: карточка «Ø Баллов» заменена на «Ø Новых клиентов», добавлена метрика «Скорость привлечения» (новые клиенты в день по выбранному периоду).
- [x] Outlets: карточка «Ø Баллов на точку» заменена на «Покупок на клиента» (повторяемость покупок).
...
- [x] Рефакторинг страниц для поддержки тем и улучшения UX:
    ...
    - `operations/page.tsx`: модальное окно деталей операции использует тот же `modal-overlay`/`modal` + размытие фона (`modal-blur-active`), что и карточка клиента.
...
## Рефактор 2025-12-05 — Единая модель Customer (per-merchant)
- [x] Обновить Prisma-схему: удалить глобальную модель `Customer`, перенести её поля в новую per-merchant модель `Customer` (бывшая `MerchantCustomer`) с обязательным `merchantId`.
- [x] Создать миграцию `20251205045807_unified_customer` и применить к БД.
- [x] Массовая замена `merchantCustomer` → `customer` в коде (~390 мест).
- [x] Переписать основные хелперы: `ensureCustomer`, `resolveCustomerContext`, `ensureCustomerByTelegram`.
- [x] Исправить дублирующиеся поля/переменные после массовой замены.
- [x] Обновить запросы: `customerId` → `id` в CustomerWhereInput.
- [x] Production код компилируется без ошибок.
- [x] Клиентские приложения (miniapp/cashier/SDK/portal) переведены на `customerId`, teleauth снова возвращает `customerId`, кеши/снапшоты ивентов читают legacy id.
- [~] Обновить тесты (spec файлы) под единую модель — 10 ошибок.
- [x] Обновить документацию (REST API, миниаппа, портал).
...
- [x] Перенумеровать RFM-баллы: `5` — самые активные клиенты, `1` — потерянные (включая агрегации, таблицы и heatmap API).
