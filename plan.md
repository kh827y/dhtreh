...
- [x] CashierGuard учитывает Bridge-подпись и данные hold: commit/refund больше не падают с 403 при требовании staff key.
...
- [x] Miniapp: добавил автоопрос истории транзакций и показ шторки отзыва после покупок (начисление/списание).
...
## 2025-?? — Мини-апп отзывы и портал (в работе)
- [x] Бэкенд: сервис отзывов без моков (листинг, настройки, публичные ручки для миниаппа).
- [x] Миниапп: шторка с оценкой после покупки, отправка отзыва, предложение поделиться.
- [x] Портал `/reviews`: реальные данные вместо моков, настройки синхронизированы с API.
- [x] Документация: README/Docs по обновлённым отзывам.
