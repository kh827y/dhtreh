...
### Текущее задание — клиентская миниаппа (редизайн)
- [x] Обновить UI миниаппы под мобильные устройства в светлой теме: карточка профиля с именем и аватаром Telegram, баланс и уровень, QR с предпросмотром и увеличением.
- [x] Добавить обязательный запрос телефона при запуске, экран первичной анкеты (имя, пол, дата рождения).
- [x] Реализовать поле ввода промокода, кнопку «Акции» со счётчиком, улучшенную историю транзакций и новый toasts/статусы.
