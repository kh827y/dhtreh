...
- [x] Клиенты: обновлён список с фильтрами, модалкой создания/редактирования и импортом CSV; карточка клиента с историей, отзывами и модалками операций; страница комплиментарных баллов; сохранён быстрый поиск по телефону из предыдущей версии.
- [x] Серверная часть портала: эндпоинты `/portal/antifraud/settings` и `/portal/customers` (листинг, карточка, операции начисления/списания, комплиментарные, отмена, импорт), хранение лимитов в `MerchantAntifraudSettings`, журнал `AntifraudAlert`, интеграция с метриками.
