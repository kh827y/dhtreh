...
- [x] Telegram Mini App: подчистил UI (состояние кнопки, skeleton для блоков, скрытие данных после отключения) и учёл предупреждение о webhook без падения подключения.
