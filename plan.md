...
- ✅ Раздел «Аудитории клиентов»: подключены реальные сегменты (`/api/portal/audiences`), форма фильтров использует данные точек/уровней, отдельная модалка состава с переходами в карточку клиента, кнопка ручного пересчёта; создание/редактирование вызывает пересчёт составов, ночной воркер ежедневно подтягивает новых клиентов под условия.
