...
- [ ] Добавить cache-first снапшот (`bootstrap`/`balance`/`transactions`) в `localStorage`/`IndexedDB` с TTL и отметкой «Обновлено …», чтобы сразу заполнять карточки до ответа API.
- [ ] Разбить `page.tsx` на независимые секции с собственными состояниями `resolved`, отказавшись от общего `screenLoading`, и отложенно подгружать необязательные блоки (промо, рефералка, настройки).
- [ ] Вывести отдельный короткий поток получения `Telegram init data` + локального `merchantCustomerId`, чтобы загружать «последние данные» ещё до завершения `teleauth`.
