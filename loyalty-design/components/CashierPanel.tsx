...
// New Interface for Checkout Client
...
const CashierPanel: React.FC<CashierPanelProps> = ({ onExit }) => {
  ...
  // Mock Data
  const defaultEmployee: EmployeeProfile = {
    id: '1',
    name: 'Алиса Ф.',
    role: 'Кассир',
    outlet: 'Центральный магазин',
    avatar: 'AF',
    pin: '1234'
  };
  const employee = currentUser ?? defaultEmployee;
  ...
  const filteredLeaderboard = useMemo(() => {
    ...
    if (ratingFilter === 'my_outlet') {
      data = data.filter(s => s.outlet === employee.outlet);
    }
    return data.sort((a, b) => b.score - a.score);
  }, [ratingFilter, employee.outlet]);

  const currentUserRating = leaderboardData.find(u => u.id === employee.id);
  ...
  const handlePinInput = (num: string) => {
    if (pin.length < 4) {
      ...
      if (newPin.length === 4) {
        const foundEmployee = staffDatabase.find(staff => staff.pin === newPin);
        if (foundEmployee) {
          setTimeout(() => {
  ...
  const handleSearchAction = (action: string) => {
    ...
    alert(`Действие: ${action} для ${inputValue || 'QR'}`);
    if (action !== 'scan') setInputValue('');
  };
  ...
  const handleReturnSearch = () => {
      ...
      if (found) {
          ...
          alert('Чек с таким номером не найден в текущей смене');
      }
  };
  ...
  const calculatePrecheck = (mode: 'accrue' | 'redeem') => {
      ...
      if (mode === 'redeem') {
          redeemed = Math.min(parseInt(txRedeemPoints) || 0, currentClient?.balance || 0, amount);
      }
      
      const final = amount - redeemed;
      // Mock logic: 5% accrual on paid amount
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden relative">
      ...
      <aside className="w-20 bg-[#1e293b] flex flex-col items-center py-6 z-30 flex-shrink-0">
         <div className="w-10 h-10 bg-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-lg mb-8 shadow-lg shadow-purple-900/30 cursor-pointer" onClick={() => handleSwitchView('main')}>
            L
         </div>
         <div className="flex-1 flex flex-col items-center space-y-4 w-full">
            <button onClick={() => handleSwitchView('main')} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${activeView === 'main' ? 'bg-purple-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:text-white hover:bg-slate-700'}`}>
               <Search size={20} />
            </button>
            <button onClick={() => handleSwitchView('history')} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${activeView === 'history' ? 'bg-purple-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:text-white hover:bg-slate-700'}`}>
               <History size={20} />
            </button>
            <button onClick={() => handleSwitchView('return')} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${activeView === 'return' ? 'bg-purple-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:text-white hover:bg-slate-700'}`}>
               <RotateCcw size={20} />
            </button>
            <button onClick={() => handleSwitchView('rating')} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${activeView === 'rating' ? 'bg-purple-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:text-white hover:bg-slate-700'}`}>
               <Award size={20} />
            </button>
         </div>
         <div className="mt-auto">
            <button onClick={onExit} className="w-10 h-10 rounded-xl text-slate-400 hover:text-red-400 hover:bg-slate-800 transition-colors flex items-center justify-center">
               <LogOut size={20} />
            </button>
         </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc]">
         ...
         <header className="h-16 px-6 flex items-center justify-between bg-white border-b border-slate-200">
            <div>
               <h1 className="font-bold text-slate-800 text-lg leading-tight">
                  {activeView === 'main' ? 'Терминал лояльности' : 
                   activeView === 'history' ? 'История операций' : 
                   activeView === 'return' ? 'Оформление возврата' : 
                   'Рейтинг сотрудников'}
               </h1>
         ...
         {activeView === 'main' ? (
                  ...
                  {checkoutStep === 'search' && (
                     <div className="space-y-4 animate-fade-in">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                           {keys.map((k) => (
                              <button key={k.val} onClick={() => { if (k.val === 'clear') handleClear(); else if (k.val === 'back') handleBackspace(); else handleNumClick(k.val); }} className={`h-16 rounded-xl text-xl font-medium transition-all duration-100 active:scale-[0.98] select-none ${k.label === 'C' ? 'bg-[#FFFBEB] text-[#D97706] border border-[#FEF3C7] hover:bg-[#FEF3C7]' : k.type === 'secondary' ? 'bg-slate-100 text-slate-500 hover:bg-slate-200' : 'bg-white text-slate-900 shadow-sm border border-slate-200 hover:border-purple-300 hover:text-purple-600'}`}>{k.label}</button>
                           ))}
                        </div>
                  ...
                  {checkoutStep === 'amount' && (
                     <div className="animate-fade-in">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                           ...
                           <div>
                              <label className="block text-sm font-medium text-gray-700 mb-2">Номер чека <span className="text-gray-400 font-normal">(необязательно)</span></label>
                              <input 
                                 type="text"
                                 value={txCheckId}
                           ...
                           <div className="flex gap-3 pt-2">
                              <button onClick={resetCheckout} className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-medium hover:bg-gray-200 transition-colors">Отмена</button>
                              <button onClick={() => { if(parseFloat(txAmount) > 0) setCheckoutStep('mode'); }} disabled={!parseFloat(txAmount)} className="flex-1 py-3 bg-purple-600 text-white rounded-xl font-medium hover:bg-purple-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">Далее</button>
                           </div>
                        </div>
                     </div>
                  )}
                  ...
                  {checkoutStep === 'mode' && (
                     <div className="animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                           <button 
                              onClick={() => { setTxRedeemPoints(''); calculatePrecheck('accrue'); }}
                           ...
                           >
                              <div className="flex items-center space-x-3 mb-2">
                                 <div className="p-2 bg-green-100 rounded-lg text-green-600 group-hover:bg-green-500 group-hover:text-white transition-colors"><Plus size={24} /></div>
                           ...
                           >
                              <div className="flex items-center space-x-3 mb-2">
                                 <div className="p-2 bg-orange-100 rounded-lg text-orange-600 group-hover:bg-orange-500 group-hover:text-white transition-colors"><Wallet size={24} /></div>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                           <div>
                              <div className="flex justify-between items-center mb-2">
                                 ...
                                 <span className="text-xs text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded">Доступно: {Math.min(currentClient.balance, parseFloat(txAmount))}</span>
                              </div>
                              <div className="relative">
                                 <input 
                                    type="number"
                              ...
                              {parseFloat(txRedeemPoints) > currentClient.balance && <p className="text-red-500 text-xs mt-1">Недостаточно баллов</p>}
                              {parseFloat(txRedeemPoints) > parseFloat(txAmount) && <p className="text-red-500 text-xs mt-1">Списание не может превышать сумму</p>}
                           </div>
                           
                           <div className="flex gap-2">
                              <button onClick={() => setTxRedeemPoints(Math.min(currentClient.balance, parseFloat(txAmount)).toString())} className="px-3 py-1.5 bg-orange-50 text-orange-700 rounded-lg text-sm font-medium hover:bg-orange-100 transition-colors">Максимум</button>
                              <button onClick={() => setTxRedeemPoints(Math.floor(Math.min(currentClient.balance, parseFloat(txAmount)) / 2).toString())} className="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">50%</button>
                           </div>

                           <div className="flex gap-3 pt-2">
                              <button onClick={() => setCheckoutStep('mode')} className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-medium hover:bg-gray-200 transition-colors">Назад</button>
                              <button 
                                 onClick={() => calculatePrecheck('redeem')} 
                                 disabled={!txRedeemPoints || parseFloat(txRedeemPoints) <= 0 || parseFloat(txRedeemPoints) > currentClient.balance || parseFloat(txRedeemPoints) > parseFloat(txAmount)} 
                        ...
                        <div className="bg-white rounded-2xl border border-purple-100 shadow-lg overflow-hidden">
                           ...
                           <div className="p-6 space-y-4">
                              <div className="flex justify-between items-center text-gray-600">
                                 ...
                                 <span className="font-medium">{fmtMoney(parseFloat(txAmount))} ₽</span>
                              </div>
                              {parseInt(txRedeemPoints) > 0 && (
                                 ...
                                 <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 my-2">
                                    ...
                                    <div className="text-left">
                                       ...
                                       <p className="text-amber-800 text-xs mt-1 leading-snug">
                                          Не забудьте уменьшить сумму чека на <strong>{txRedeemPoints} ₽</strong> в вашей POS-системе перед оплатой.
                                       </p>
                                    </div>
                                 </div>
                              )}
                           ...
                           <div className="p-4 bg-gray-50 flex gap-3">
                              ...
                              <button onClick={() => setCheckoutStep('success')} className="flex-1 py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-colors shadow-md flex items-center justify-center space-x-2">
                     ...
                     <div className="animate-fade-in flex flex-col items-center justify-center h-full">
                        ...
                        <div className="w-full max-w-sm bg-white rounded-t-2xl shadow-xl overflow-hidden relative pb-2 mb-6">
                           ...
                           <div className="bg-emerald-500 p-6 text-center text-white relative overflow-hidden">
                              ...
                              <p className="text-emerald-100 text-sm">{new Date().toLocaleString('ru-RU', {day: 'numeric', month:'long', hour:'2-digit', minute:'2-digit'})}</p>
                           </div>
                           ...
                           <div className="p-6 bg-white relative z-10 space-y-5">
                              ...
                              <div className="space-y-3 mb-6">
                                 <div className="flex justify-between items-center text-sm">
                                    ...
                                    <span className="font-bold text-slate-900">{fmtMoney(parseFloat(txAmount))} ₽</span>
                                 </div>
                                 {parseInt(txRedeemPoints) > 0 && (
                                    <div className="flex justify-between items-center text-sm">
                                       <span className="text-orange-500 flex items-center"><Coins size={14} className="mr-1"/> Списано баллов</span>
                                       <span className="font-bold text-orange-500">-{txRedeemPoints}</span>
                                    </div>
                                 )}
                                 {txAccruePoints > 0 && (
                                    <div className="flex justify-between items-center text-sm">
                                       <span className="text-emerald-600 flex items-center"><Gift size={14} className="mr-1"/> Начислено</span>
                        ...
                        {/* Action Button */}
                        <button onClick={resetCheckout} className="w-full max-w-sm py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg active:scale-[0.98] flex items-center justify-center">
                           Закрыть
                        </button>
                     </div>
                  )}
            ...
            // ... (rest of return view code unchanged) ...
            // === VIEW 3: RETURNS (Redesigned) ===
                  ...
                  {returnTx ? (
                     <div className="bg-white rounded-2xl shadow-xl shadow-red-900/5 border border-red-100 overflow-hidden animate-fade-in">
                        ...
                        <div className="p-6 space-y-6">
                           <div className="grid grid-cols-2 gap-4 text-sm">
                              <div><span className="block text-gray-500 text-xs uppercase font-semibold mb-1">Номер чека</span><div className="font-mono font-medium text-gray-900">{returnTx.checkId}</div></div>
                              <div><span className="block text-gray-500 text-xs uppercase font-semibold mb-1">Дата продажи</span><div className="font-medium text-gray-900">{returnTx.date.toLocaleDateString()}</div></div>
                              <div className="col-span-2 pt-2 border-t border-gray-100"><span className="block text-gray-500 text-xs uppercase font-semibold mb-1">Клиент</span><div className="font-medium text-gray-900 flex items-center"><User size={16} className="mr-2 text-gray-400" />{returnTx.client}</div></div>
                           </div>
                           <div className="bg-gray-50 rounded-xl p-4 space-y-3 border border-gray-200">
                              <div className="flex justify-between items-center"><span className="text-gray-600 font-medium">Сумма возврата</span><span className="text-lg font-bold text-gray-900">{fmtMoney(returnTx.amount)} ₽</span></div>
                              <div className="h-px bg-gray-200"></div>
                              {returnTx.pointsAccrued > 0 && <div className="flex justify-between items-center text-sm"><span className="text-gray-500">Будет списано</span><span className="font-bold text-red-600">-{returnTx.pointsAccrued} Б</span></div>}
                              {returnTx.pointsRedeemed !== 0 && <div className="flex justify-between items-center text-sm"><span className="text-gray-500">Будет возвращено</span><span className="font-bold text-green-600">+{Math.abs(returnTx.pointsRedeemed)} Б</span></div>}
                           </div>
                           <div className="flex space-x-3 pt-2">
                              <button onClick={() => { setReturnTx(null); setReturnSearchInput(''); }} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 transition-colors">Отмена</button>
                              <button onClick={handleConfirmReturn} className="flex-1 py-3 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 shadow-md transition-colors flex items-center justify-center space-x-2"><RotateCcw size={18} /><span>Выполнить возврат</span></button>
                           </div>
                        </div>
                     </div>
                  ) : (
                     <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                        <div className="flex flex-col items-center mb-8">
                           <div className="w-14 h-14 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center mb-4 shadow-sm shadow-red-100"><RotateCcw size={28} /></div>
                           <h2 className="text-xl font-bold text-slate-900">Оформление возврата</h2>
                           <p className="text-slate-500 text-sm mt-1 text-center">Введите номер чека для поиска транзакции</p>
                        </div>
                        <div className="space-y-4">
                           <div className="relative">
                              <input type="text" value={returnSearchInput} onChange={(e) => setReturnSearchInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleReturnSearch()} placeholder="№ Чека" className="w-full bg-slate-50 border border-slate-200 text-slate-900 font-medium rounded-xl px-4 py-3 pl-11 focus:bg-white focus:border-red-500 focus:ring-4 focus:ring-red-500/10 transition-all outline-none" autoFocus />
                              <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                           </div>
                           <button onClick={handleReturnSearch} disabled={!returnSearchInput} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-red-600/20 disabled:opacity-50 disabled:shadow-none transition-all flex items-center justify-center space-x-2 active:scale-[0.98]"><span>Найти операцию</span></button>
                        </div>
                     </div>
                  )}
               </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 flex-shrink-0">
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center justify-between relative overflow-hidden">
                           ...
                           <div className="relative z-10">
                              ...
                              <div className="flex items-baseline mt-2 space-x-2">
                                 ...
                                 <span className="text-sm font-medium text-yellow-600 bg-yellow-100 px-2 py-0.5 rounded-full">{currentUserRating?.rank}-е место</span>
                              </div>
                              <p className="text-xs text-slate-400 mt-2 flex items-center"><Clock size={12} className="mr-1" /> Период: {motivationSettings.period}</p>
                           </div>
                           <div className="relative z-10 p-3 bg-yellow-100 rounded-full text-yellow-600 shadow-sm"><Trophy size={32} /></div>
                        </div>
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-center">
                           ...
                           <div className="space-y-3">
                              <div className="flex justify-between items-center"><div className="flex items-center space-x-2 text-slate-700 text-sm"><UserPlus size={16} className="text-blue-500" /><span>Новый клиент</span></div><span className="font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded text-xs">+{motivationSettings.pointsNew} очков</span></div>
                              <div className="flex justify-between items-center"><div className="flex items-center space-x-2 text-slate-700 text-sm"><User size={16} className="text-purple-500" /><span>Постоянный клиент</span></div><span className="font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded text-xs">+{motivationSettings.pointsExisting} очков</span></div>
                           </div>
                        </div>
                     </div>
                     <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 flex-shrink-0">
                           ...
                           <div className="flex bg-white rounded-lg p-1 border border-slate-200 shadow-sm">
                              <button onClick={() => setRatingFilter('all')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${ratingFilter === 'all' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:bg-slate-100'}`}>Вся сеть</button>
                              <button onClick={() => setRatingFilter('my_outlet')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${ratingFilter === 'my_outlet' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:bg-slate-100'}`}>Моя точка</button>
                           </div>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                           {filteredLeaderboard.map((user, index) => {
                              const isMe = user.id === employee.id;
                              const currentRank = index + 1;
                              return (
                                 <div key={user.id} className={`flex items-center p-3 rounded-xl mb-2 transition-colors ${isMe ? 'bg-purple-50 border border-purple-100' : 'hover:bg-slate-50 border border-transparent'}`}>
                                    <div className="w-10 flex-shrink-0 flex justify-center">{currentRank === 1 ? <div className="w-8 h-8 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center shadow-sm"><Crown size={16} fill="currentColor" /></div> : currentRank === 2 ? <div className="w-8 h-8 bg-gray-100 text-slate-500 rounded-full flex items-center justify-center shadow-sm font-bold text-sm">2</div> : currentRank === 3 ? <div className="w-8 h-8 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center shadow-sm font-bold text-sm">3</div> : <span className="text-slate-400 font-medium text-sm">{currentRank}</span>}</div>
                                    <div className="flex items-center flex-1 ml-4"><div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs mr-3 ${isMe ? 'bg-purple-600 text-white shadow-md shadow-purple-200' : 'bg-slate-200 text-slate-500'}`}>{user.avatar}</div><div><div className="flex items-center"><span className={`font-bold text-sm ${isMe ? 'text-purple-900' : 'text-slate-900'}`}>{user.name}</span>{isMe && <span className="ml-2 text-[10px] bg-purple-200 text-purple-800 px-1.5 rounded font-bold">Вы</span>}</div><div className="text-xs text-slate-500 flex items-center mt-0.5"><Store size={10} className="mr-1" />{user.outlet}</div></div></div>
                                    <div className="text-right"><span className={`font-bold text-lg ${isMe ? 'text-purple-700' : 'text-slate-700'}`}>{user.score}</span><span className="text-[10px] text-slate-400 block uppercase font-medium">очков</span></div>
                                 </div>
                              );
                           })}
                        </div>
            ...
            <main className="flex-1 flex flex-col bg-slate-50/50 p-6 overflow-hidden">
               {/* ... (rest of history view code unchanged) ... */}
               <div className="max-w-5xl w-full mx-auto h-full flex flex-col">
                  ...
                  <div className="flex flex-col space-y-4 mb-6 flex-shrink-0">
                     ...
                     <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center gap-3">
                        <div className="relative flex-1 w-full lg:w-auto"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="№ Чека или Имя клиента..." value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 outline-none transition-all" /></div>
                        <div className="relative w-full lg:w-40"><input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="w-full pl-3 pr-2 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 outline-none transition-all text-slate-600" /></div>
                        <div className="flex bg-slate-100 p-1 rounded-lg w-full lg:w-auto">{(['all', 'sale', 'return'] as const).map((type) => (<button key={type} onClick={() => setFilterType(type)} className={`flex-1 lg:flex-none px-3 py-1.5 text-xs font-semibold rounded-md transition-all capitalize ${filterType === type ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{type === 'all' ? 'Все' : type === 'sale' ? 'Продажа' : 'Возврат'}</button>))}</div>
                        <div className="relative w-full lg:w-48"><select value={filterStaff} onChange={(e) => setFilterStaff(e.target.value)} className="w-full pl-3 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 outline-none transition-all text-slate-600 appearance-none cursor-pointer"><option value="all">Все сотрудники</option>{uniqueStaff.map(s => <option key={s} value={s}>{s}</option>)}</select><User size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" /></div>
                     </div>
                  </div>
                  {/* History List */}
                     ...
                     <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex-1 flex flex-col">
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                           {paginatedHistory.length === 0 ? (
                              <div className="flex flex-col items-center justify-center h-full text-slate-400"><Search size={48} className="mb-4 opacity-20" /><p>Операции не найдены</p><button onClick={() => { setHistorySearch(''); setFilterType('all'); setFilterDate(''); setFilterStaff('all'); }} className="mt-2 text-purple-600 hover:underline text-sm">Сбросить фильтры</button></div>
                           ) : (
                              <div className="divide-y divide-slate-100">
                                 {paginatedHistory.map((tx) => {
                                    const isReturn = tx.type === 'return';
                                    return (
                                       <div key={tx.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors group">
                                          <div className="flex items-start sm:items-center w-full sm:w-auto"><div className={`w-10 h-10 rounded-lg flex items-center justify-center mr-4 flex-shrink-0 border ${isReturn ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'}`}>{isReturn ? <RotateCcw size={18} /> : <Receipt size={18} />}</div><div><div className="flex items-center gap-2"><span className="font-bold text-slate-900 text-base">{fmtMoney(tx.amount)} ₽</span>{isReturn && <span className="bg-red-100 text-red-700 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">Возврат</span>}</div><div className="flex items-center text-xs text-slate-500 mt-1 relative"><button onClick={() => handleCopy(tx.id, tx.checkId)} className="flex items-center space-x-1 bg-slate-100 hover:bg-slate-200 px-1.5 py-0.5 rounded transition-colors mr-2 group/copy max-w-[180px]" title="Копировать"><Copy size={10} className="text-slate-400 group-hover/copy:text-slate-600 flex-shrink-0" /><span className="font-mono truncate">{tx.checkId}</span></button><span>{tx.date.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })}, {tx.date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</span>{copiedId === tx.id && <span className="absolute -bottom-4 left-0 text-[10px] text-green-600 font-medium bg-white/90 backdrop-blur-sm px-1.5 py-0.5 rounded border border-green-100 shadow-sm animate-fade-in z-10 pointer-events-none">Скопировано!</span>}</div></div></div>
                                          <div className="flex flex-col w-full sm:w-1/3 text-left sm:px-4"><div className="flex items-center text-sm font-medium text-slate-900 mb-1"><User size={14} className="mr-2 text-slate-400" /><span className="truncate">{tx.client}</span></div><div className="flex items-center text-xs text-slate-500"><span className="bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100 truncate max-w-[150px]">Кассир: {tx.staff}</span></div></div>
                                          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto min-w-[100px] border-t sm:border-0 border-slate-50 pt-2 sm:pt-0">{tx.pointsAccrued !== 0 && <div className={`flex items-center text-sm font-bold ${tx.pointsAccrued > 0 ? 'text-green-600' : 'text-red-500'}`}>{tx.pointsAccrued > 0 ? '+' : ''}{tx.pointsAccrued} Б {tx.pointsAccrued > 0 ? <ArrowUpRight size={14} className="ml-1" /> : <ArrowDownLeft size={14} className="ml-1" />}</div>}{tx.pointsRedeemed !== 0 && <div className={`flex items-center text-sm font-bold ${tx.pointsRedeemed > 0 ? 'text-red-500' : 'text-green-600'}`}>{tx.pointsRedeemed > 0 ? '-' : '+'}{Math.abs(tx.pointsRedeemed)} Б {tx.pointsRedeemed > 0 ? <ArrowDownLeft size={14} className="ml-1" /> : <ArrowUpRight size={14} className="ml-1" />}</div>}{tx.pointsAccrued === 0 && tx.pointsRedeemed === 0 && <span className="text-xs text-slate-400">Без баллов</span>}</div>
                                       </div>
                                    );
                                 })}
                              </div>
