...
export class LoyaltyApi {
  ...
  // Endpoints
  quote(body: { mode: 'redeem'|'earn'; merchantId: string; userToken: string; orderId: string; total: number; eligibleTotal: number; outletId?: string; deviceId?: string; staffId?: string }, opts?: { staffKey?: string; bridgeSignatureSecret?: string }) {
  ...
  commit(body: { merchantId: string; holdId: string; orderId: string; receiptNumber?: string }, opts?: { idempotencyKey?: string; bridgeSignatureSecret?: string }) {
    const json = JSON.stringify(body);
    const headers: any = {};
