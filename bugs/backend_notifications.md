...
- **[SEC][TENANT] Email-рассылки позволяют отправлять письма клиентам другого мерчанта**
  - **Риск**: интегратор с валидным `X-Api-Key` может указать `customerIds` чужого мерчанта и отправить письма/контент не своим клиентам (пересечение аудиторий, утечки ПДн/спам). Это нарушение изоляции мерчантов.
  - **Где**:
    - `api/src/notifications/email/email.service.ts`: `sendCampaignEmail()` выбирает клиентов только по `id`, без фильтра `merchantId`.
    - `api/src/notifications/email/email.service.ts`: `sendWelcomeEmail()` не проверяет, что `customer.merchantId === merchantId`.
  - **Что сделать**: фильтровать клиентов по `{ id, merchantId }` и/или явно проверять принадлежность клиента мерчанту перед отправкой.
...
- **[DATA] `GET /push/stats/:merchantId` принимает любые `from/to` без валидации дат**
  - **Риск**: `new Date('invalid')` даёт `Invalid Date` → возможные ошибки в Prisma, некорректные метрики, нестабильные ответы.
  - **Где**: `api/src/notifications/push/push.controller.ts` — парсинг `from/to` без проверки валидности.
  - **Что сделать**: валидировать даты и возвращать 400 при неверном формате.
