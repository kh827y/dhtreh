...
1) **Секреты интеграций доступны любому сотруднику с правами на чтение любой механики**
   - **Где:** `api/src/portal/portal.controller.ts` (метод `assertSettingsReadAccess()` и `GET /portal/settings`), `api/src/merchants/dto.ts` (`MerchantSettingsRespDto` содержит `webhookSecret`, `bridgeSecret`, `telegramBotToken` и т.д.).
   - **Что происходит:** доступ к `/portal/settings` разрешён, если у сотрудника есть *любое* право из списка (например, `mechanic_birthday`/`antifraud`). При этом ответ возвращает чувствительные поля (`webhookSecret`, `bridgeSecret`, `telegramBotToken`, `bridgeSecretNext` и т.д.).
   - **Риск:** утечка секретов у низкоуровневых ролей → возможность подделки вебхуков/bridge‑подписей, компрометация Telegram‑бота и интеграций.
   - **Почему это важно:** в портале лояльности много “узких” ролей (маркетинг/механики), им обычно **не** нужно видеть секреты.

2) **PII клиентов доступна через промо‑акции без прав на клиентов**
...
3) **PII клиентов доступна через уровни лояльности без прав на клиентов**
...
4) **В уровнях лояльности нет верхних ограничений на проценты начисления/списания**
...
5) **`GET /portal/loyalty/mechanics` может падать на невалидном статусе**
