...
- **[Points Burn] Возможен некорректный “двойной” списание баллов при гонках и расходовании lot-ов**
  - **Риск**: если лоты уже частично израсходованы конкурентными операциями, воркер всё равно списывает из кошелька полный `burnReq`, что может привести к списанию баллов, которые не должны сгореть. Это прямой риск потери баллов клиентом и спорных списаний.
  - **Причина**: внутри транзакции `toBurn` уменьшается по мере обновления `earnLot`, но фактическое списание из кошелька рассчитывается заново как `min(wallet.balance, burnReq)` и **не зависит** от реально «сожжённых» лотов. При конкурентных обновлениях `earnLot` остатки могли уменьшиться, но кошелёк всё равно уменьшится на исходный `burnReq`.
  - **Где**: `api/src/points-burn.worker.ts` (расчёт `toBurn`/`burnAmount`, обновление `earnLot` и `wallet`).
  - **Что сделать**: привязать списание к фактически «сожжённой» сумме (например, считать `burnAmount = (burnReq - toBurn)` после прохода по лотам) и/или блокировать `earnLot`/`wallet` на уровне БД в транзакции (row-level lock), чтобы исключить гонки.
