...
### 4) HoldGcWorker и IdempotencyGcWorker полностью подавляют ошибки
- **Риск:** при проблемах с БД/Prisma воркеры могут «тихо» не выполнять GC, оставляя `HOLD` в `PENDING` и накапливая idempotency‑ключи. В проде это приводит к росту нагрузки и странному поведению (например, зависшие холды/повторные ответы на запросы).
- **Причина:** `tick()` вызывается через `setInterval(...).catch(() => {})`, а внутренние ошибки в обновлениях/удалениях также проглатываются; логирования/алертов нет.
- **Где смотреть:** `api/src/hold-gc.worker.ts`, `api/src/idempotency-gc.worker.ts`.
- **Что сделать:** хотя бы логировать ошибки (или увеличивать метрику ошибок), чтобы можно было увидеть деградацию GC.
