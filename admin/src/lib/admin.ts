...
async function http<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (method !== 'GET' && method !== 'HEAD' && !('x-admin-action' in mergedHeaders)) {
    mergedHeaders['x-admin-action'] = 'ui';
  }
...
// Paged lists for CRM
export async function listTransactionsAdmin(merchantId: string, params: { limit?: number; before?: string; from?: string; to?: string; type?: string; customerId?: string; outletId?: string; staffId?: string }): Promise<{ items: Record<string, unknown>[]; nextBefore: string | null }> {
  const p = new URLSearchParams();
  ...
  if (params.staffId) p.set('staffId', params.staffId);
  const qs = p.toString();
  const res = await http<unknown>(`/merchants/${encodeURIComponent(merchantId)}/transactions${qs?`?${qs}`:''}`);
  const resRecord = isRecord(res) ? res : null;
  const rawItems = Array.isArray(resRecord?.items) ? resRecord?.items : Array.isArray(res) ? res : [];
  const items = rawItems.filter(isRecord);
  const nextBeforeRaw = resRecord?.nextBefore;
  const nextBefore: string | null = nextBeforeRaw ? String(nextBeforeRaw) : (items.length ? String(items[items.length - 1]?.createdAt || '') : null);
  return { items, nextBefore: nextBefore || null };
}

export async function listReceiptsAdmin(merchantId: string, params: { limit?: number; before?: string; orderId?: string; customerId?: string }): Promise<Record<string, unknown>[]> {
  const p = new URLSearchParams();
  if (params.limit != null) p.set('limit', String(params.limit));
  if (params.before) p.set('before', params.before);
  if (params.orderId) p.set('orderId', params.orderId);
  if (params.customerId) p.set('customerId', params.customerId);
  const qs = p.toString();
  const res = await http<unknown>(`/merchants/${encodeURIComponent(merchantId)}/receipts${qs?`?${qs}`:''}`);
  return Array.isArray(res) ? res.filter(isRecord) : [];
}
...
export async function createOutlet(merchantId: string, dto: { name: string }): Promise<Outlet> {
...
export async function getCustomerTimeline(merchantId: string, customerId: string, limit = 50): Promise<{ items: Array<{ type: string; at: string; data: unknown }> }> {
  const p = new URLSearchParams();
  if (limit != null) p.set('limit', String(limit));
  const qs = p.toString();
...
export async function createDefaultSegments(merchantId: string): Promise<{ ok: true } | Record<string, unknown>> {
  return http(`/segments/merchant/${encodeURIComponent(merchantId)}/defaults`, { method: 'POST' });
}

export async function recalcSegment(segmentId: string): Promise<{ ok: true } | Record<string, unknown>> {
