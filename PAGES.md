...
- **Уволить** — делает пользователя неактивным: доступы блокируются, история сохраняется.
## Доступы
- **Доступ в админ панель** — тумблер (включен по умолчанию для владельца мерчанта, для него выключить нельзя).  
    Вкл → пользователь может входить в админку. Выкл → вход запрещён, но доступ в панель кассира может оставаться активным.
...
Каждому созданному сотруднику присваивается уникальный пинкод из 4 цифр, при входе в виртуальный терминал кассира он должен будет вводить свой пинкод (система идентифицирует кассира по этому пинкоду). Помимо пинкода сначала кассиру выводится окно авторизации: логин и пароль. Логин - это название мерчанта (отформатированное к нижнему регистру без пробелов и без прочих символов не являющихся английскими буквами), а пароль - это 9 цифр. В окне авторизации поле для ввода пароля будет в виде 9 квадратов, где каждые 3 квадрата отделены "-". Информация о логине и пароле для входа в панель кассира будет отображаться в настройках мерчант панели, пароль возможно перегенерировать кнопкой вращающихся стрелочек. 
