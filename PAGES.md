...
- **Чекбокс:** «Не показывать значения меньше … процентов». При включении активируется поле ввода (дефолт — «3») и скрываются столбцы, где доля клиентов ≤ указанного порога.
- **График:** «Покупок на покупателя» — столбчатая диаграмма с долей клиентов (в %) по количеству покупок на 1 уникального клиента.
## Динамика
- **Заголовок:** «Динамика».
- **Фильтры:
    - Выбор периода.
...
Текстовое поле Текст сообщения при нажатии «Отправить сообщение» (textarea, 0/300; используется миниаппой при вызове Telegram share и передаётся другу).
Подсказки-плейсхолдеры (для обоих текстов):
...
- Валидации: уникальность телефона, совпадение паролей, корректный email/дата.
...
- Блок «Профиль» (таблица ключ-значение): Телефон, Email, Имя, Бонусных баллов, Отложенных баллов, Пол, Возраст, Дата рождения, Дней с последнего визита, Частота визитов, Средний чек, Сумма покупок (прошлый/текущий месяц и за всё время), Теги, Дата регистрации, Комментарий к пользователю, Блокировка начислений, Пригласивший (с переходом в карточку).
