...
describe('TTL full flow (lots + PENDING -> activation -> preview -> burn)', () => {
  ...
  const mkTx = () => ({
    earnLot: {
      findUnique: jest.fn(async (args: any) => state.lots.find(l => l.id === args.where.id) || null),
      findMany: jest.fn(async (args: any) => {
        let arr = state.lots.filter(l => l.merchantId === args.where.merchantId);
        if (args.where.status) arr = arr.filter(l => l.status === args.where.status);
        if (args.where.earnedAt?.lt) arr = arr.filter(l => l.earnedAt && l.earnedAt < args.where.earnedAt.lt);
        if (args.where.consumedPoints?.gt != null) arr = arr.filter(l => (l.consumedPoints||0) > args.where.consumedPoints.gt);
        if (args.orderBy?.earnedAt === 'asc') arr = arr.sort((a,b)=> (a.earnedAt?.getTime()||0) - (b.earnedAt?.getTime()||0));
        if (args.orderBy?.earnedAt === 'desc') arr = arr.sort((a,b)=> (b.earnedAt?.getTime()||0) - (a.earnedAt?.getTime()||0));
        if (args.orderBy?.maturesAt === 'asc') arr = arr.sort((a,b)=> (a.maturesAt?.getTime()||0) - (b.maturesAt?.getTime()||0));
    ...
    wallet: {
      ...
      findUnique: jest.fn(async () => ({ id: state.wallet.id, balance: state.wallet.balance })),
    ...
    eventOutbox: { create: jest.fn(async (args: any) => { createdEvents.push(args.data); return args.data; }) },
  });

  const prisma: any = {
    merchantSettings: {
      findMany: jest.fn(async (_args: any) => [{ merchantId: 'M1', pointsTtlDays: 30 }]),
    },
    earnLot: {
    ...
    eventOutbox: {
      create: jest.fn(async (args: any) => { createdEvents.push(args.data); return args.data; }),
    },
  ...
  it('activates PENDING lot -> creates EARN and wallet increases', async () => {
    ...
    const w = new EarnActivationWorker(prisma as any, metrics);
    // @ts-ignore private
    await (w as any).tick();
  ...
  it('TTL preview emits expiringPoints for ACTIVE lot older than cutoff', async () => {
    ...
    const w = new PointsTtlWorker(prisma as any, metrics);
    // @ts-ignore private
    await (w as any).tick();

    const ev = createdEvents.find(e => e.eventType === 'loyalty.points_ttl.preview');
  ...
  it('TTL burn consumes lots and reduces wallet balance', async () => {
    ...
    const burned = createdEvents.find(e => e.eventType === 'loyalty.points_ttl.burned');
