...
describe('CRM (e2e)', () => {
  ...
  const state = {
    customers: [
      { id: 'C1', phone: '+79990000001' },
      { id: 'C2', phone: '+79990000002' },
    ],
  ...
  const prismaMock: any = {
    ...
    customer: {
      findFirst: async (args: any) =>
        state.customers.find((c) => c.phone === args.where.phone) || null,
    },
    wallet: {
      findFirst: async (args: any) =>
        state.wallets.find(
    ...
    transaction: {
      findMany: async (args: any) => {
        let arr = state.txns.filter(
          (t) => t.merchantId === args.where.merchantId,
        );
        if (args.where.customerId)
          arr = arr.filter((t) => t.customerId === args.where.customerId);
        if (args.where.before?.lt)
          arr = arr.filter((t) => t.createdAt < args.where.before.lt);
    ...
    receipt: {
      findMany: async (args: any) => {
        let arr = state.receipts.filter(
          (r) => r.merchantId === args.where.merchantId,
        );
        if (args.where.customerId)
          arr = arr.filter((r) => r.customerId === args.where.customerId);
        if (args.orderBy?.createdAt === 'desc')
          arr = arr
            .slice()
            .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
