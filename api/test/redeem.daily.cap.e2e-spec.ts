...
describe('REDEEM daily cap (e2e)', () => {
  ...
  const prismaMock: any = {
    ...
    merchantSettings: { findUnique: async (args: any) => ({ merchantId: args.where.merchantId, earnBps: 500, redeemLimitBps: 5000, redeemDailyCap: 700, updatedAt: new Date(), rulesJson: null }) },
    ...
    transaction: {
      findMany: async (_args: any) => {
        ...
        const parts = [] as any[];
        let rem = state.used;
        while (rem > 0) { const chunk = Math.min(rem, 250); parts.push({ amount: -chunk, type: 'REDEEM', createdAt: new Date() }); rem -= chunk; }
        return parts;
      }
    },
    wallet: {
  ...
  it('Daily cap limits redeem to 300 when used=400 and limit=700', async () => {
    state.walletBal = 10000; state.used = 400;
    const res = await request(app.getHttpServer())
      .post('/loyalty/quote')
      .send({ merchantId: 'M-DCAP', userToken: 'C1', mode: 'REDEEM', orderId: 'ORD-1', total: 1000, eligibleTotal: 1000 })
      .expect(201);
  ...
  it('Daily cap exhausted -> deny redeem', async () => {
    state.walletBal = 10000; state.used = 700;
    const res = await request(app.getHttpServer())
      .post('/loyalty/quote')
      .send({ merchantId: 'M-DCAP', userToken: 'C1', mode: 'REDEEM', orderId: 'ORD-2', total: 1000, eligibleTotal: 1000 })
      .expect(201);
