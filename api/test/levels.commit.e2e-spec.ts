  ...
  const prismaMock: any = {
    ...
    transaction: {
      findMany: async (args: any) =>
        state.txns.filter(
          (t) =>
            t.merchantId === args.where.merchantId &&
            t.customerId === args.where.customerId &&
            (!args.where.type || t.type === args.where.type) &&
      ...
      update: async (args: any) => {
        for (const [k, v] of state.wallets.entries())
          if (v.id === args.where.id) {
            ...
            return { id: v.id, balance: v.balance };
          }
