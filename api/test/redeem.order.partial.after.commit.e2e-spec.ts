...
describe('REDEEM partial after commit — remaining cap on same order (e2e)', () => {
  ...
  const prismaMock: any = {
    ...
    transaction: {
      findMany: async (_args: any) => state.txns.filter((t: any)=>t.type==='REDEEM'),
      create: async (args: any) => { state.txns.push({ id: 'T1', ...args.data, createdAt: new Date() }); return { id: 'T1' }; },
    },
    wallet: {
      findFirst: async (_args: any) => ({ id: 'W1', balance: state.walletBal }),
      create: async (args: any) => ({ id: 'W1', ...args.data, balance: state.walletBal }),
  ...
  it('Quote (wallet-limited 300) -> Commit -> Quote same order returns remaining 200', async () => {
    const q1 = await request(app.getHttpServer())
      .post('/loyalty/quote')
      .send({ merchantId: 'M-OPCAP', userToken: 'C1', mode: 'REDEEM', orderId: 'ORD-P1', total: 1000, eligibleTotal: 1000 })
      .expect(201);
