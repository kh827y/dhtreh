...
describe('CSV Exports (e2e)', () => {
  ...
  const prismaMock: any = {
    ...
    receipt: {
      findMany: async (args: any) => {
        ...
        if (args.where.createdAt?.lt)
          arr = arr.filter((x) => x.createdAt < args.where.createdAt.lt);
        if (args.where.orderId)
          arr = arr.filter((x) => x.orderId === args.where.orderId);
        if (args.where.customerId)
          arr = arr.filter((x) => x.customerId === args.where.customerId);
    ...
    transaction: {
      findMany: async (args: any) => {
        let arr = state.transactions.filter(
          (x) =>
            x.merchantId === args.where.merchantId &&
            x.customerId === args.where.customerId,
        );
        if (args.where.createdAt?.lt)
          arr = arr.filter((x) => x.createdAt < args.where.createdAt.lt);
        if (args.orderBy?.createdAt === 'desc')
  ...
  it('GET /merchants/:id/receipts.csv returns CSV with header', async () => {
    ...
    expect(res.text.split('\n')[0]).toContain(
      'id,orderId,customerId,total,eligibleTotal,redeemApplied,earnApplied,createdAt,outletId,staffId',
    );
  });
