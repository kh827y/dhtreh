...
describe('ReferralController (e2e, mocked Prisma)', () => {
  ...
  const prismaMock: any = {
    ...
    merchant: {
      findUnique: jest.fn(async () => ({ id: 'M1', name: 'M1' })),
  ...
  it('POST /referral/program returns 400 when active program exists', async () => {
    jest.spyOn(prismaMock.referralProgram, 'findFirst').mockResolvedValueOnce({
      id: 'EXIST',
      merchantId: 'M1',
      status: 'ACTIVE',
    });
  ...
  it('POST /referral/program returns 201 when no active program', async () => {
    jest
      .spyOn(prismaMock.referralProgram, 'findFirst')
  ...
  it('POST /referral/activate returns 201 and welcome message on valid code', async () => {
    jest.spyOn(prismaMock.referral, 'findFirst').mockResolvedValueOnce({
      id: 'R1',
      programId: 'P1',
      referrerId: 'C1',
      code: 'GOOD',
      ...
      program: { merchantId: 'M1', refereeReward: 10 },
    });
    const res = await request(app.getHttpServer())
      .post('/referral/activate')
      .set('x-api-key', 'test-key')
