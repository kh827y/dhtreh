...
describe('AntiFraud Guard (e2e)', () => {
  ...
  const prismaMock: any = {
    ...
    merchantSettings: {
      findUnique: async (args: any) =>
        state.merchantSettings.get(args.where.merchantId) || null,
    ...
    hold: {
      ...
      create: async (args: any) => {
        const h = { id: args.data.id || uuid(), ...args.data };
        state.holds.push(h);
        return h;
      },
      update: async (args: any) => {
    ...
    transaction: {
      findMany: async (args: any) => {
        ...
        return state.transactions
          .filter((t) => {
            if (where.merchantId && t.merchantId !== where.merchantId)
              return false;
    ...
    adminAudit: {
      create: async (args: any) => {
        state.audits.push({ id: uuid(), ...args.data });
        return state.audits[state.audits.length - 1];
      },
  ...
  it('CRITICAL риск -> блокировка по скорингу', async () => {
    ...
    for (let i = 0; i < 7; i++) {
      state.transactions.push({
        ...
        merchantId: 'M-af2',
        customerId: 'C-crit',
        createdAt: new Date(ts.getTime() - 60 * 1000),
        outletId: null,
        staffId: null,
      });
    }
  ...
  it('Merchant velocity: блокировка при превышении лимита по мерчанту', async () => {
    ...
    state.merchantSettings.set('M-cap', {
      ...
      rulesJson: {
        af: {
          merchant: { limit: 1, windowSec: 3600, dailyCap: 0, weeklyCap: 0 },
          outlet: { limit: 999, windowSec: 3600 },
          staff: { limit: 999, windowSec: 3600 },
          customer: { limit: 999, windowSec: 3600 },
        },
      },
      updatedAt: new Date(),
    });
  ...
  it('Refund outlet limit: блокировка рефанда по лимиту торговой точки', async () => {
    ...
    state.merchantSettings.set('M-ref', {
      ...
      rulesJson: {
        af: {
          merchant: { limit: 999, windowSec: 3600 },
          outlet: { limit: 0, windowSec: 3600 },
          staff: { limit: 999, windowSec: 3600 },
          customer: { limit: 999, windowSec: 3600 },
        },
      },
