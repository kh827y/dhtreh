...
export class OperationsLogService {
  ...
  async list(
    ...
    const transactionDisplayCount = includeTransactions
      ...
      : 0;
    const totalCount =
      (includeReceipts ? receiptCount : 0) + transactionDisplayCount;
  ...
  private describeTransaction(
    ...
    const change = Number(tx.amount ?? 0) || 0;
  ...
  private mergeEarnRedeemTransactions(
    ...
    for (const item of items) {
      ...
      if (
        new Date(item.occurredAt).getTime() > new Date(current.latest).getTime()
      ) {
        current.latest = item.occurredAt;
        current.base = item;
      }
