...
type LoyaltyServicePrivate = {
  ensureCustomerContext: (input: unknown) => Promise<CustomerContext>;
  getSettings: (merchantId: string) => Promise<MerchantSettings>;
  isAllowSameReceipt: (merchantId: string) => Promise<boolean>;
  resolvePositions: (
    merchantId: string,
    items: PositionInput[],
...
describe('LoyaltyService redeem caps', () => {
  ...
  const stubIntegrationContext = (service: LoyaltyService) => {
    ...
    jest.spyOn(servicePrivate, 'ensureCustomerContext').mockResolvedValue({
      customerId: 'c-test',
    ...
    jest.spyOn(servicePrivate, 'isAllowSameReceipt').mockResolvedValue(true);
  };
  ...
  it('skips earn when redeem requested and allowSameReceipt=false', async () => {
    ...
    jest.spyOn(servicePrivate, 'isAllowSameReceipt').mockResolvedValue(false);
    jest.spyOn(servicePrivate, 'resolvePositions').mockResolvedValue([
      {
        qty: 1,
        price: 100,
