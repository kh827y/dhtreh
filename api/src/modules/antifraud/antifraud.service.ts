...
export class AntiFraudService {
  ...
  private async checkTime(merchantId: string) {
    ...
    const local = new Date(Date.now() + timezone.utcOffsetMinutes * 60 * 1000);
  ...
  private async checkOutlet(context: TransactionContext) {
    ...
    const uniqueCount = uniqueOutlets.filter((item) =>
  ...
  private async checkBlacklist(
    ...
    return Boolean(customer?.accrualsBlocked || customer?.redemptionsBlocked);
  }
  ...
  private async logSuspiciousActivity(
    ...
    try {
      ...
      if (level === RiskLevel.CRITICAL) {
        this.sendAdminAlert(context, score, factors);
      }
    } catch (error) {
  ...
  async getStatistics(merchantId: string, days = 30) {
    ...
    const blocked = audits.filter((audit) => {
      const payload = this.asRecord(audit.payload);
      return payload?.riskLevel === RiskLevel.CRITICAL;
    }).length;
    const reviewed = audits.filter((audit) => {
      const payload = this.asRecord(audit.payload);
      return payload?.riskLevel === RiskLevel.HIGH;
    }).length;
    ...
    for (const audit of audits) {
      const payload = this.asRecord(audit.payload);
      const arr = Array.isArray(payload?.factors) ? payload.factors : [];
  ...
  async recordFraudCheck(
    ...
    try {
      ...
      const fraudCheckDelegate = prisma.fraudCheck;
      if (!fraudCheckDelegate) {
        // In unit/e2e tests PrismaService may be partially mocked without fraudCheck
        return null;
      }
      const rec = await fraudCheckDelegate.create({
        data: {
          merchantId: context.merchantId,
          customerId: context.customerId,
