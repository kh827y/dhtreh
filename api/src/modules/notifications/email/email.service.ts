...
type EmailSendInfo = { messageId?: string | null };
type EmailTransporter = {
  sendMail: (options: EmailSendOptions) => Promise<EmailSendInfo>;
};
...
export class EmailService {
  ...
  constructor(
    ...
    const createTransportSafe = (
      nodemailer as unknown as {
        createTransport: (cfg: SmtpConfig) => EmailTransporter;
      }
    ).createTransport;
    this.transporter = createTransportSafe(smtpConfig);
    this.defaultFrom =
  ...
  async sendEmail(dto: SendEmailDto): Promise<boolean> {
    try {
      ...
      // Настройки письма
      const mailOptions: EmailSendOptions = {
