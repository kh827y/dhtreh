...
import { ALLOW_INACTIVE_SUBSCRIPTION_KEY } from '../../core/guards/subscription.guard';
import {
  hasPortalPermission,
...
type PortalRequest = {
  ...
  portalStaffName?: string | null;
  portalAccessGroups?: Array<{
    id: string;
    name: string | null;
    scope: string | null;
  }>;
  portalPermissions?: PortalPermissionState | null;
  portalSubscription?: SubscriptionState;
  portalTimezone?: string;
  portalTimezoneOffsetMinutes?: number;
  portalTimezoneIana?: string;
};
...
export class PortalGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const auth = String(this.getHeader(req, 'authorization') || '');
      ...
      if (claims.actor === 'STAFF') {
        ...
        if (!adminImpersonation) {
          ...
          if (this.isTokenRevoked(issuedAt, staff.portalTokensRevokedAt))
            return false;
          if (
            this.isTokenRevoked(issuedAt, staff.merchant?.portalTokensRevokedAt)
          )
            return false;
        }
  ...
  private resolvePermissionTarget(req: PortalRequest) {
  ...
  private enforcePortalPermissions(
    req: PortalRequest,
  ...
  private getHeader(req: PortalRequest, name: string): string | undefined {
