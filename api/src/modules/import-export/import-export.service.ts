...
export class ImportExportService {
      ...
      try {
        ...
        if (hasAggregates) {
          if (totalSpent === null || visitsCount === null) {
            throw new Error(
              'total_spent и visits_count обязательны для агрегатов',
            );
          }
        }
        ...
        const rowResult = await this.prisma.$transaction(async (tx) => {
          ...
          if (hasOperation) {
            ...
            if (cached) {
              ...
              if (!same) {
                throw new Error(
                  'order_id уже используется для другой операции',
                );
              }
              receiptsSkipped++;
            } else {
  ...
  private parseCsv(buffer: Buffer): RowRecord[] {
  ...
  private parseExcel(buffer: Buffer): RowRecord[] {
  ...
  private generateCsv(data: Array<Record<string, unknown>>): Buffer {
    if (data.length === 0) {
      return Buffer.from('');
    }
  ...
  private toTrimmedString(value: unknown): string | null {
    const raw = this.stringifyCsvValue(value).trim();
  ...
  private generateExcel(data: Array<Record<string, unknown>>): Buffer {
