...
export class TelegramMiniappGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const req = context.switchToHttp().getRequest<TelegramMiniappRequest>();
  ...
  private collectCustomerCandidates(req: TelegramMiniappRequest): string[] {
    const sources = [
      req.body?.customerId,
  ...
  private collectCustomerIdentifiers(req: TelegramMiniappRequest): string[] {
    const fields = [
      req.body?.customerId,
      req.query?.customerId,
      req.params?.customerId,
