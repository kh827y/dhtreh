...
export class PortalTelegramNotifyService {
  constructor(private readonly prisma: PrismaService, private readonly notify: TelegramNotifyService) {}
  ...
  async getState(merchantId: string) {
    ...
    const botUsername = botInfo?.username ? (botInfo.username.startsWith('@') ? botInfo.username : `@${botInfo.username}`) : null;
  ...
  async issueInvite(merchantId: string, forceNew?: boolean) {
    if (!merchantId) throw new BadRequestException('merchantId required');
    const botInfo = await this.notify.getBotInfo();
    if (!botInfo?.username) throw new BadRequestException('Бот уведомлений не настроен (см. админ-панель)');
    ...
    if (!forceNew) {
      invite = await prismaAny.telegramStaffInvite.findFirst({
        where: {
          merchantId,
          OR: [
            { expiresAt: null },
    ...
    if (!invite) {
      invite = await prismaAny.telegramStaffInvite.create({
        data: { merchantId, token: this.genToken(), expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
      });
    }
  ...
  async listSubscribers(merchantId: string) {
    if (!merchantId) throw new BadRequestException('merchantId required');
    const prismaAny = this.prisma as any;
    const rows = await prismaAny.telegramStaffSubscriber.findMany({ where: { merchantId, isActive: true }, orderBy: { addedAt: 'desc' } });
  ...
  async deactivateSubscriber(merchantId: string, subscriberId: string) {
    ...
    const prismaAny = this.prisma as any;
    const existing = await prismaAny.telegramStaffSubscriber.findUnique({ where: { id: subscriberId } }).catch(() => null);
    if (!existing || existing.merchantId !== merchantId) throw new NotFoundException('subscriber not found');
    await prismaAny.telegramStaffSubscriber.update({ where: { id: subscriberId }, data: { isActive: false } });
