...
export class LevelsController {
  ...
  @ApiOkResponse({ schema: { type: 'object', properties: {
    ...
    value: { type: 'number' },
    current: { type: 'object', properties: { name: { type: 'string' }, threshold: { type: 'number' } } },
    next: { type: 'object', nullable: true, properties: { name: { type: 'string' }, threshold: { type: 'number' } } },
    progressToNext: { type: 'number' },
  } } })
  async current(@Param('merchantId') merchantId: string, @Param('customerId') customerId: string) {
