...
describe('AnalyticsService.getAutoReturnMetrics', () => {
  const prisma: any = {
    ...
    autoReturnAttempt: { findMany: jest.fn() },
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(service as any, 'getTimezoneInfo')
  ...
  it('считает приглашения, возвраты, баллы и выручку с учетом сгорания', async () => {
    ...
    const dayInvites = result.trends.attempts.find((row) => row.date === '2025-01-02');
    const dayReturns = result.trends.attempts.find((row) => row.date === '2025-01-03');

    expect(dayInvites).toEqual({ date: '2025-01-02', invitations: 1, returns: 0 });
    expect(dayReturns).toEqual({ date: '2025-01-03', invitations: 1, returns: 1 });
  });
});
