...
export class AntiFraudGuard implements CanActivate {
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    let limits = {
      ...
      outlet:   { limit: envNum('AF_LIMIT_OUTLET', 20), windowSec: envNum('AF_WINDOW_OUTLET_SEC', 600), dailyCap: envNum('AF_DAILY_CAP_OUTLET', 0),   weeklyCap: envNum('AF_WEEKLY_CAP_OUTLET', 0) },
    ...
    const block = (scope: string, count: number, limit: number) => {
      ...
      try {
        this.alerts
          .antifraudBlocked({ merchantId, reason: 'velocity', scope, ctx: { customerId, outletId: resolvedOutletId, staffId } })
          .catch(() => {});
      } catch {}
          ...
          if (hold && hold.customerId && hold.merchantId) {
            ...
            try {
              ...
              if (shouldApplyNoOutlet) {
                ...
                try {
                  this.alerts
                    .antifraudBlocked({ merchantId: hold.merchantId, reason: 'factor', factor, ctx: { customerId: hold.customerId, outletId: hold.outletId || undefined, staffId: hold.staffId } })
                    .catch(() => {});
                } catch {}
            ...
            if (score.shouldBlock) {
              ...
              try {
                this.alerts
                  .antifraudBlocked({ merchantId: hold.merchantId, reason: 'risk', level: String(score.level), ctx: { customerId: hold.customerId, outletId: hold.outletId || undefined, staffId: hold.staffId } })
                  .catch(() => {});
              } catch {}
            ...
            if (blockFactors.length && Array.isArray(score.factors)) {
              ...
              if (matched) {
                ...
                try {
                  this.alerts
                    .antifraudBlocked({ merchantId: hold.merchantId, reason: 'factor', factor: matched, ctx: { customerId: hold.customerId, outletId: hold.outletId || undefined, staffId: hold.staffId } })
                    .catch(() => {});
                } catch {}
