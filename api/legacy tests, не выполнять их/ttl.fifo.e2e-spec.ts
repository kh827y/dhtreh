...
describe('TTL FIFO burn with multiple ACTIVE lots (e2e-like, mock Prisma)', () => {
    ...
    earnLot: {
      findMany: jest.fn(async (args: any) => {
        ...
        if (args.where.earnedAt?.lt)
          arr = arr.filter((l) => l.earnedAt < args.where.earnedAt.lt);
  ...
  const prisma: any = {
    merchantSettings: {
      findMany: jest.fn(async () => [{ merchantId: 'M1', pointsTtlDays: 30 }]),
    },
    earnLot: {
      findMany: jest.fn(async (args: any) => {
        ...
        let arr = state.lots.filter(
          (l) =>
            l.merchantId === args.where.merchantId && l.status === 'ACTIVE',
        );
        if (args.where.earnedAt?.lt)
          arr = arr.filter((l) => l.earnedAt < args.where.earnedAt.lt);
        return arr.map((x) => ({ ...x }));
  ...
  it('burns FIFO and updates consumedPoints and wallet', async () => {
    ...
    const burned = state.events.find(
