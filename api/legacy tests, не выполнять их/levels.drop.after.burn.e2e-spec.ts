...
describe('Levels drop after period window (+TTL burn) (e2e)', () => {
  ...
  const prismaMock: any = {
      ...
      findMany: async () => [
        {
          ...
          pointsTtlDays: state.settings.pointsTtlDays,
        },
      ],
    },
    ...
    transaction: {
      async findMany(args: any) {
        const since = args?.where?.createdAt?.gte
          ? new Date(args.where.createdAt.gte)
          : undefined;
    ...
    wallet: {
      findFirst: async (_args: any) =>
        state.walletBal != null ? { id: 'W1', balance: state.walletBal } : null,
    ...
    $transaction: async (fn: (tx: any) => Promise<any>) =>
      fn({
        ...
        wallet: {
          findFirst: async (_args: any) =>
            state.walletBal != null
              ? { id: 'W1', balance: state.walletBal }
              : null,
          create: async (args: any) => ({ id: 'W1', ...args.data }),
          findUnique: async (_args: any) => ({
            id: 'W1',
  ...
  it('Initially Silver -> quote 70', async () => {
    const r1 = await request(app.getHttpServer())
      .get(`/levels/${state.settings.merchantId}/C-LDROP`)
      .expect(200);
