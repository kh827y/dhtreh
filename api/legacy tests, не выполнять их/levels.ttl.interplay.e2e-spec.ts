...
describe('Levels x TTL interplay (e2e)', () => {
  ...
  const prismaMock: any = {
    ...
    merchantSettings: {
      findUnique: async (args: any) =>
        state.settings.get(args.where.merchantId) || null,
    ...
    earnLot: {
      findMany: async (args: any) => {
        ...
        let arr = state.lots.filter(
          (l) =>
            (!where.merchantId || l.merchantId === where.merchantId) &&
            (!where.customerId || l.customerId === where.customerId),
        );
    ...
    transaction: {
      findMany: async (args: any) => {
        ...
        let arr = state.txns.filter(
          (t) => t.merchantId === w.merchantId && t.customerId === w.customerId,
        );
        if (w.type) arr = arr.filter((t) => String(t.type) === String(w.type));
        if (w.createdAt?.gte)
          arr = arr.filter((t) => t.createdAt >= new Date(w.createdAt.gte));
        return arr.map((x) => ({ ...x }));
      },
  ...
  beforeAll(async () => {
    ...
    const act = new EarnActivationWorker(prismaMock, {
      inc: () => {},
      setGauge: () => {},
    } as any);
    // @ts-ignore private
