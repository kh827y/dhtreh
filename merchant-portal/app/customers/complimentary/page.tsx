...
export default function ComplimentaryPointsPage() {
  ...
  const canSubmit = !!customerId && form.amount.trim().length > 0 && form.expiresIn.trim().length > 0 && !submitting;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!validate()) return;
    if (!customerId) {
      setApiError("Не удалось определить клиента. Откройте страницу из карточки клиента.");
      return;
    }
    setSubmitting(true);
    try {
      const res = await fetch(`/api/portal/customers/${encodeURIComponent(customerId)}/complimentary`, {
        method: "POST",
      ...
      window.setTimeout(() => {
        router.push(`/customers/${customerId}#operations`);
      }, 500);
    } catch (err) {
      setApiError(err instanceof Error ? err.message : "Ошибка начисления");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <CardBody>
          <form onSubmit={handleSubmit} style={{ display: "grid", gap: 20, maxWidth: 520 }}>
              ...
              <label style={fieldStyle}>
                ...
                {!customerId && (
                  <span style={{ fontSize: 12, color: "#f87171" }}>
                    Укажите клиента, открыв форму из карточки.
                  </span>
                )}
              </label>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <Link href={customerId ? `/customers/${customerId}` : "/customers"} style={{ color: "#94a3b8", alignSelf: "center" }}>
                Отмена
              </Link>
            </div>
          </form>
