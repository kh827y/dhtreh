  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Импорт завершился ошибкой");
      }
      const json = (await res.json()) as { processed: number; created: number; updated: number; errors: ErrorRow[] };
      if (json.errors?.length) {
        setErrors(json.errors);
      } else {
        setMessage(`Файл ${file.name} обработан. Создано ${json.created}, обновлено ${json.updated} записей.`);
      }
    } catch (err) {
      setErrors([{ line: 0, column: "—", message: err instanceof Error ? err.message : "Ошибка импорта" }]);
    } finally {
      setImporting(false);
    }
  }
