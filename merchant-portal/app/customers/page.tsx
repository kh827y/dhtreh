...
const { Search, RefreshCw, ChevronLeft, ChevronRight, Plus, UploadCloud } = Icons;
...
export default function CustomersPage() {
  ...
  const customerGroups = React.useMemo(() => {
    const base = new Set<string>(["Стандарт", "Постоянные", "VIP", "Новые"]);
    items.forEach((item) => {
  ...
  async function handleCreateCustomer(payload: CustomerFormPayload) {
    try {
      ...
      const response = await fetch("/api/portal/customers", {
        ...
        body: JSON.stringify({
          ...
          tags: payload.tags ? payload.tags.split(/[;,]/).map((tag) => tag.trim()).filter(Boolean) : undefined,
      ...
      if (!response.ok) {
        const message = await response.text();
        throw new Error(message || "Не удалось создать клиента");
      }
      setToast("Клиент создан");
      setModalOpen(false);
      setRefreshToken((token) => token + 1);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err ?? "Не удалось создать клиента");
        ...
        <CardHeader
          ...
          actions={
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <Button type="button" leftIcon={<Plus size={16} />} onClick={() => setModalOpen(true)}>
                Добавить клиента
              </Button>
              <Button
                type="button"
