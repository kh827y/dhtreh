...
const ReferralSettings: React.FC<ReferralSettingsProps> = ({ onNavigate }) => {
  // Settings State
  const [settings, setSettings] = useState({
    isEnabled: true,
    
    // Referrer Logic
    rewardTrigger: 'first', // 'first' | 'all'
    rewardType: 'fixed', // 'fixed' | 'percent'
    isMultiLevel: false,
    levels: [
      { level: 1, value: 100 }, // Direct invite
      { level: 2, value: 50 },  // Friend of friend
      { level: 3, value: 25 }   // Friend of friend of friend
    ],
    minOrderAmount: 0,
    ...
    // Messaging
    inviteCtaText: 'Расскажите друзьям о нашей программе и получите бонус. Делитесь ссылкой {link} или вашим кодом {code}.',
    shareMessageText: 'Переходите по ссылке {link} и получите {bonusamount} бонусов на баланс в программе лояльности {businessname}'
  });
  ...
  const updateLevelValue = (index: number, val: number) => {
    const newLevels = [...settings.levels];
    newLevels[index].value = val;
    setSettings({ ...settings, levels: newLevels });
  };

  const insertPlaceholder = (field: 'inviteCtaText' | 'shareMessageText', placeholder: string) => {
    setSettings(prev => ({
      ...prev,
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-fade-in">
      ...
      <div className="space-y-6">
         ...
         {/* Hero Status Card */}
         <div className={`rounded-xl border transition-colors ${settings.isEnabled ? 'bg-purple-50 border-purple-200' : 'bg-white border-gray-200'}`}>
            <div className="p-6 flex items-center justify-between">
               <div className="flex items-start space-x-4">
                  <div className={`p-3 rounded-lg ${settings.isEnabled ? 'bg-white text-purple-600 shadow-sm' : 'bg-gray-100 text-gray-400'}`}>
            ...
            <div className="xl:col-span-7 space-y-6">
               ...
               <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                           <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">За что поощрять?</label>
                        ...
                        <div>
                           <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Тип поощрения</label>
                           ...
                           <div className="mt-4">
                              <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Мин. сумма заказа</label>
                        ...
                        <div className="space-y-3">
                           {settings.isMultiLevel ? (
                              ...
                              settings.levels.map((lvl, index) => (
                                 <div key={lvl.level} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-100 animate-fade-in">
               ...
               <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Приветственные баллы</label>
                        <div className="relative">
                           <input 
                              type="number" 
                              min="0"
                        ...
                        <p className="text-xs text-gray-400 mt-2">
                           Начисляются сразу после регистрации по реферальной ссылке.
                        </p>
                     </div>
                        ...
                        <label className="flex items-start space-x-3 cursor-pointer p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                           ...
                           <div>
                              ...
                              <p className="text-xs text-gray-500 mt-1 leading-snug">
                                 Если включено: <strong>150</strong> (за рег.) + <strong>{settings.friendReward}</strong> (реф.) = <strong>{150 + settings.friendReward}</strong> баллов.
                              </p>
                           </div>
                        </label>
                     </div>
            ...
            <div className="xl:col-span-5 space-y-6">
               ...
               <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-3">
                     ...
                     <div className="flex flex-wrap gap-2">
                        <button onClick={() => insertPlaceholder('inviteCtaText', '{businessname}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Название компании</button>
                        <button onClick={() => insertPlaceholder('inviteCtaText', '{bonusamount}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Бонус приглашающему</button>
                        <button onClick={() => insertPlaceholder('inviteCtaText', '{code}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Код</button>
                        <button onClick={() => insertPlaceholder('inviteCtaText', '{link}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Ссылка</button>
                     </div>
                  </div>
               </div>
               ...
               <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-3">
                     ...
                     <div className="flex flex-wrap gap-2">
                        <button onClick={() => insertPlaceholder('shareMessageText', '{businessname}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Название компании</button>
                        <button onClick={() => insertPlaceholder('shareMessageText', '{bonusamount}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Награда другу</button>
                        <button onClick={() => insertPlaceholder('shareMessageText', '{link}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Ссылка</button>
                        <button onClick={() => insertPlaceholder('shareMessageText', '{code}')} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-[10px] font-medium text-gray-600 rounded border border-gray-200">Код</button>
                     </div>
                  </div>

                  <div className="mt-4 bg-teal-50 p-3 rounded-lg flex items-start space-x-2 text-xs text-teal-800">
                     <Info size={14} className="mt-0.5 flex-shrink-0" />
                     <p>Это сообщение будет автоматически подставлено в мессенджер, когда клиент нажмет кнопку «Отправить сообщение».</p>
                  </div>
               </div>
